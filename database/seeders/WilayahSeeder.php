...
{
    /**
     * Seed data wilayah dengan fokus utama Provinsi Aceh:
     * - 38 Provinsi di Indonesia
     * - 23 Kabupaten/Kota LENGKAP di Provinsi Aceh (18 Kabupaten, 5 Kota)
     * - Kecamatan dan Gampong/Desa sentra perikanan pesisir di Aceh
     */
    public function run(): void
    {
        // 1. Data 38 Provinsi Indonesia
        $provinces = [
            ['code' => '11', 'name' => 'Aceh'],
        ...
        // =====================================================================
        // FOKUS: PROVINSI ACEH (Kode BPS/Kemendagri: 11)
        // 23 Kabupaten/Kota Lengkap
        // =====================================================================
        ...
        if ($aceh) {
            ...
            // --- KECAMATAN & GAMPONG PESISIR DAN SENTRA PERIKANAN ACEH ---

            // 1. Kota Banda Aceh (11.71) - Sentra Pelabuhan Perikanan Samudera (PPS) Lampulo
            ...
            if ($bandaAceh) {
                ...
                foreach ($districtsBA as $d) {
                    $dist = District::updateOrCreate(['code' => $d['code']], ['regency_id' => $bandaAceh->id, 'name' => $d['name']]);

                    // Gampong pesisir di Kuta Raja (Lokasi PPS Lampulo)
                    if ($d['code'] === '11.71.02') {
                        $gampongs = [
                        ...
                        foreach ($gampongs as $g) {
                            Village::updateOrCreate(['code' => $g['code']], ['district_id' => $dist->id, 'name' => $g['name'], 'postal_code' => $g['postal_code']]);
                        }
                    }

                    // Gampong pesisir di Meuraxa (Pelabuhan Ulee Lheue)
                    if ($d['code'] === '11.71.01') {
                        $gampongs = [
                    ...
                    // Gampong pesisir di Syiah Kuala (Krueng Cut / Alue Naga)
                    if ($d['code'] === '11.71.05') {
                        $gampongs = [
            ...
            // 2. Kabupaten Aceh Besar (11.06) - Wilayah pesisir & kepulauan Pulo Aceh
            ...
            if ($acehBesar) {
                ...
                foreach ($districtsAB as $d) {
                    $dist = District::updateOrCreate(['code' => $d['code']], ['regency_id' => $acehBesar->id, 'name' => $d['name']]);
                    ...
                    if ($d['code'] === '11.06.21') { // Pulo Aceh (sentra perikanan pulau terluar)
                        $gampongs = [
            ...
            // 3. Kota Sabang (11.72) - Pulau Weh (WPP 572 Selat Malaka & Samudera Hindia)
            ...
            if ($sabang) {
                ...
                foreach ($districtsSabang as $d) {
                    $dist = District::updateOrCreate(['code' => $d['code']], ['regency_id' => $sabang->id, 'name' => $d['name']]);
                    if ($d['code'] === '11.72.01') {
                        $gampongs = [
            ...
            // 4. Kabupaten Aceh Timur (11.03) - Sentra PPN / PPI Kuala Idi
            $acehTimur = Regency::where('code', '11.03')->first();
            if ($acehTimur) {
                $dist = District::updateOrCreate(['code' => '11.03.04'], ['regency_id' => $acehTimur->id, 'name' => 'Idi Rayeuk']);
            ...
            // 5. Kabupaten Aceh Barat (11.05) - Sentra PPI Ujong Baroh Meulaboh
            $acehBarat = Regency::where('code', '11.05')->first();
            if ($acehBarat) {
                $dist = District::updateOrCreate(['code' => '11.05.01'], ['regency_id' => $acehBarat->id, 'name' => 'Johan Pahlawan']);
                Village::updateOrCreate(
                    ['code' => '11.05.01.2001'],
            ...
            // 6. Kabupaten Simeulue (11.17) - Kepulauan Penghasil Lobster & Tuna
            $simeulue = Regency::where('code', '11.17')->first();
            if ($simeulue) {
                $dist = District::updateOrCreate(['code' => '11.17.01'], ['regency_id' => $simeulue->id, 'name' => 'Simeulue Timur']);
                Village::updateOrCreate(
                    ['code' => '11.17.01.2001'],
            ...
            // 7. Kabupaten Aceh Singkil (11.18) - Kepulauan Banyak & Pesisir Singkil
            $singkil = Regency::where('code', '11.18')->first();
            if ($singkil) {
                $dist = District::updateOrCreate(['code' => '11.18.01'], ['regency_id' => $singkil->id, 'name' => 'Singkil']);
                Village::updateOrCreate(
                    ['code' => '11.18.01.2001'],
